...
void Symbol::setSymbolType(SymbolType new_type) {
    if (new_type != SYM_TYPE_UKN && new_type != SYM_TYPE_FUNC &&
...
void Symbol::setName(std::string new_name) { this->name = new_name; }

void Symbol::setAddr(std::uint64_t new_addr) { this->addr = new_addr; }

Symbol::SymbolType Symbol::getSymbolType() { return this->type; }

const char* Symbol::getName() { return this->name.c_str(); }

std::uint64_t Symbol::getAddr() { return this->addr; }

/*
 *   Section functions
 */

Section::Section() : type(SEC_TYPE_NONE), vma(0), size(0), bytes(NULL) {
...
void Section::setNewName(std::string new_name) { this->name = new_name; }

void Section::setNewSectionType(SectionType new_type) {
    if (new_type != SEC_TYPE_NONE && new_type != SEC_TYPE_CODE &&
...
void Section::setNewVMA(std::uint64_t new_vma) { this->vma = new_vma; }

void Section::setNewSize(std::uint64_t new_size) { this->size = new_size; }

void Section::setNewBytes(std::uint64_t size) {
    this->bytes = (std::uint8_t*)malloc(size);
}

Binary* Section::getBinary() { return this->binary.get(); }

const char* Section::getName() { return this->name.c_str(); }

Section::SectionType Section::getSectionType() { return this->type; }

std::uint64_t Section::getVMA() { return this->vma; }

std::uint64_t Section::getSize() { return this->size; }

std::uint8_t* Section::getBytes() { return this->bytes; }
...
Section* Binary::get_text_sections() {
    for (auto& s : sections) {
        if (strcmp(s.getName(), ".text") == 0) return &s;
    }
    return nullptr;
}


void Binary::setFileName(const char* filename) { this->filename = filename; }

void Binary::setType(BinaryType type) { this->type = type; }

void Binary::setTypeStr(const char* type_str) {
    this->type_str = std::string(type_str);
}

void Binary::setBinaryArch(BinaryArch arch) { this->arch = arch; }

void Binary::setBinaryArchStr(const char* arch_str) {
    this->arch_str = std::string(arch_str);
}

void Binary::setBits(std::uint32_t bits) { this->bits = bits; }

void Binary::setEntryPoint(std::uint64_t entry) { this->entry = entry; }

const char* Binary::getFileName() { return this->filename.c_str(); }

Binary::BinaryType Binary::getType() { return this->type; }

const char* Binary::getTypeStr() { return this->type_str.c_str(); }

Binary::BinaryArch Binary::getBinaryArch() { return this->arch; }

const char* Binary::getBinaryArchStr() { return this->arch_str.c_str(); }

std::uint32_t Binary::getBits() { return this->bits; }

std::uint64_t Binary::getEntryPoint() { return this->entry; }

std::vector<Section>& Binary::getSections() { return this->sections; }

std::vector<Symbol>& Binary::getSymbols() { return this->symbols; }

/*
 *   Loader functions
 */
Loader::Loader(const char* file_name, Binary::BinaryType bin_type)
    : bfd_inited(false), fname(file_name), bfd_h(nullptr), type(bin_type) {
 ...
 *   public functions
 */
void Loader::load_binary() { load_binary_bfd(); }
...
Binary* Loader::getBinary() { return this->bin.get(); }
...
void Loader::open_bfd() {
    ...
    if (!bfd_check_format(bfd_h, bfd_object)) {
        snprintf(error_message, 999,
                 "file '%s' does not look like an executable (%s)\n",
                 fname.c_str(), bfd_errmsg(bfd_get_error()));
    ...
    /*
     *   Some bfd_check_format versions set wrong_format error even before
     * detecting the format and then neglect to unset this error value once the
     * format has been detected. Unset it manually to prevent problems
     */
...
void Loader::load_binary_bfd() {
    ...
    bin->setEntryPoint(
        static_cast<std::uint64_t>(bfd_get_start_address(bfd_h)));
...
void Loader::load_symbols_bfd() {
    ...
    if (n < 0) {
        ...
        for (i = 0; i < nsyms; i++) {
            if (std::find(weak_names.begin(), weak_names.end(),
                          std::string(bfd_symtab[i]->name)) !=
                weak_names.end()) {
                remove_symbol_by_name(bfd_symtab[i]->name);
                weak_names.erase(
                    std::remove(weak_names.begin(), weak_names.end(),
                                std::string(bfd_symtab[i]->name)),
                    weak_names.end());
            }
            ...
            if (bfd_symtab[i]->flags & BSF_FUNCTION) {
                ...
                sym->setAddr(static_cast<std::uint64_t>(
    ...
    if (n < 0) {
        ...
        if (!bfd_dynsym)
            throw exception_t::error(
                "Not possible to allocate memory for dynamic symbols");
...
void Loader::load_sections_bfd() {
    ...
    for (bfd_sec = bfd_h->sections; bfd_sec; bfd_sec = bfd_sec->next) {
        ...
        if (!bfd_get_section_contents(bfd_h, bfd_sec, sec->getBytes(), 0,
                                      size)) {
