...
int main(int argc, char *argv[]) {
    ...
    if (argc != 6) {
        ...
        puts("Inject the file <inject> into the given <elf>, using\n");
        puts("the given <name> and base <addr>. You can optionally specify\n");
        puts("an offset to a new <entry> point (-1 if none)\n");
        return 1;
    }
...
int rewrite_code_section(elf_data_t *elf, inject_data_t *inject) {
    ...
    while ((scn = elf_nextscn(elf->e, scn))) {
        ...
        if (!strcmp(s, ".note.ABI-tag")) {
            shdr.sh_name = shdr.sh_name;               /* 字符串表偏移 */
            shdr.sh_type = SHT_PROGBITS;               /* 类型 */
            shdr.sh_flags = SHF_ALLOC | SHF_EXECINSTR; /* 标记 */
            shdr.sh_addr = inject->secaddr; /* 加载section的地址 */
            shdr.sh_offset = inject->off;   /* section的文件偏移 */
            shdr.sh_size = inject->len;     /* 注入的字节大小 */
            shdr.sh_link = 0;               /* 代码节不用 */
            shdr.sh_info = 0;               /* 代码节不用 */
            shdr.sh_addralign = 16;         /* 内存对齐 */
            shdr.sh_entsize = 0;            /* 代码节不用 */
...
int write_shdr(elf_data_t *elf, Elf_Scn *scn, GElf_Shdr *shdr, size_t sidx) {
    ...
    off = lseek(elf->fd, elf->ehdr.e_shoff + sidx * elf->ehdr.e_shentsize,
                SEEK_SET);
    if (off < 0) {
        fprintf(stderr, "lseek failed\n");
        return -1;
...
int rewrite_section_name(elf_data_t *elf, inject_data_t *inject) {
    ...
    while ((scn = elf_nextscn(elf->e, scn))) {
        ...
        if (!strcmp(s, ".note.ABI-tag")) {
            stroff = shdr.sh_name; /* shstrtab表中的偏移 */
        } else if (!strcmp(s, ".shstrtab")) {
            strbase = shdr.sh_offset; /* shstrtab在文件中的偏移 */
        }
    }
...
int rewrite_code_segment(elf_data_t *elf, inject_data_t *inject) {
    inject->phdr.p_type = PT_LOAD;          /* 类型 */
    inject->phdr.p_offset = inject->off;    /* segment的文件偏移 */
    inject->phdr.p_vaddr = inject->secaddr; /* 加载segment的虚拟地址 */
    inject->phdr.p_paddr = inject->secaddr; /* 加载segment的物理地址 */
    inject->phdr.p_filesz = inject->len;    /* 文件的字节大小 */
    inject->phdr.p_memsz = inject->len;     /* 内存的字节大小 */
    inject->phdr.p_flags = PF_R | PF_X;     /* 标记 */
    inject->phdr.p_align = 0x1000;          /* 内存和文件对齐 */
...
int write_phdr(elf_data_t *elf, inject_data_t *inject) {
    ...
    off =
        lseek(elf->fd, elf->ehdr.e_phoff + inject->pidx * elf->ehdr.e_phentsize,
              SEEK_SET);
    if (off < 0) {
        fprintf(stderr, "lseek failed\n");
        return -1;
...
int reorder_shdrs(elf_data_t *elf, inject_data_t *inject) {
    ...
    skip = 0;
    for (scn = elf_getscn(elf->e, inject->sidx + direction); scn != NULL;
